...
from funchat.extensions import socketio, db
from funchat.forms import ProfileForm
from funchat.models import Message, User
from funchat.utils import to_html, flash_errors
	...
	emit(
		'new message',
		{
			'message_html': render_template('chat/_anonymous_message.html',
							message=html_message, avatar=avatar, nickname=nickname),
			'message_body': html_message,
			'gravatar': avatar,
			'nickname': nickname,
			'user_id': current_user.id
...
@chat_bp.route('/')
def home():
    amount = current_app.config['FUNCHAT_MESSAGE_PER_PAGE']
...
def get_messages():
	...
	pagination = Message.query.order_by(Message.timestamp.desc()).paginate(
		page,
		per_page=current_app.config['FUNCHAT_MESSAGE_PER_PAGE'],
	)
